...
pub const Token = struct {
    ...
    pub fn toString(this: Token, src: []const u8) []const u8 {
        return switch (this.kind) {
            .number, .identifier, .string => src[this.start..this.end],
...
const Tokenizer = struct {
    ...
    fn readString(this: *Tokenizer) void {
        const open = this.nextByte();
        if (open != '"' and open != '\'') {
            std.debug.panic("invalid string opener: {c}", .{open});
        }

        while (true) {
            const char = this.nextByte();
            if (char == 0 or char == open) return;
        }
    }
